...
def render_flip_board(text, stay_sec=4.0):
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .screw::before {{ width: 10px; height: 2px; }} /* 十字橫線 */
        .screw::after {{ width: 2px; height: 10px; }}  /* 十字直線 */
    ...
    <script>
        ...
        window.onload = () => {{
            ...
            if(pages.length > 1) setInterval(rotate, {safe_stay * 1000});
        }};
    </script>
    </body>
    </html>
    ...
    # 使用整數化的 timestamp 作為 key，確保 key 絕對是字串
    component_key = f"flap_v12_{int(time.time())}"
    components.html(str(html_code), height=850, scrolling=False, key=component_key)
