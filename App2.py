...
with st.container():
    ...
    if st.button("🚀 套用設定並隱藏", use_container_width=True):
        st.rerun()
...
# --- 5. 渲染顯示 ---
st.markdown("<h2 style='text-align: center; color: #333; font-family: Microsoft JhengHei; margin-top: 30px;'>𓃥 白六新聞 / 訊息告示牌</h2>", unsafe_allow_html=True)

render_flip_board(json.dumps(processed_list), stay_sec=stay_sec)

st.markdown(f"<p style='text-align: center; color: #888;'>模式: {mode} | 總計 {len(processed_list)-1} 則輪播</p>", unsafe_allow_html=True)
